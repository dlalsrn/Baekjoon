from decimal import *
getcontext().prec = 150
getcontext().rounding = ROUND_HALF_UP
pi = Decimal('3.')

def sin_(x):
	x = x % (2 * pi)
	i, lasts, s, fact, num, sign = Decimal(str('1')), Decimal(str('0')), Decimal(str(x)), Decimal(str('1')), Decimal(str(x)), Decimal(str('1'))
	while s != lasts:
		lasts = s
		i += 2
		fact *= i * (i - 1)
		num *= x * x
		sign *= -1
		s += num / fact * sign
	return Decimal(str(s))

A, B, C = map(int, input().split())
a = Decimal('0.0')
b = Decimal('1000000.0')
mid = Decimal(str(Decimal(str(a)) + Decimal(str(b)))) / Decimal('2')
fx = Decimal(str(Decimal(str(A))*Decimal(str(mid)))) + Decimal(str(Decimal(str(B))*Decimal(str(sin_(mid))))) - Decimal(str(C))

while Decimal(abs(fx)) > Decimal('0.0000000000000001'):
    if fx > 0 :
        b = Decimal(str(mid))
    elif fx < 0:
        a = Decimal(str(mid))
    else:
        break
    mid = mid = Decimal(str(Decimal(str(a)) + Decimal(str(b)))) / Decimal('2')
    fx = Decimal(str(Decimal(str(A))*Decimal(str(mid)))) + Decimal(str(Decimal(str(B))*Decimal(str(sin_(mid))))) - Decimal(str(C))

print('{:.6f}'.format(Decimal(str(mid))))