// 남극 탐험
// 22.07.25
// 엣지 or 정점간 경로의 유무(경로가 존재한다면 경로 상에 있는 값의 합)를 출력하는 문제
// 미리 엣지를 입력받아서 분리 집합으로 사이클을 제거한 최종 트리를 구성해놓고 명령을 순서대로 처리
#include <iostream>
#include <vector>
#include <cmath>
using namespace std;
...
vector <pair<string, pair<int, int>>> com;
vector <bool> visit;
int N, M, cnt;
...
{
	if (start == end)
		segtree[n] = value[start + 1].second;
	else
	{
	...
	for (int i = 0; i < M; i++)
	{
		...
		if (ch == "excursion")
		{
			...
			else
			{
				...
				sum += get_sum(1, 0, N - 1, section[t], section[t]);
				cout << sum << '\n';
			}
		}
		else if (ch == "bridge")
