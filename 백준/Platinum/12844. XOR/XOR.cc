...
ll XOR(vector<pair<ll, ll>>& tree, ll n, ll start, ll end, ll left, ll right)
{
	lazy(tree, n, start, end);
	...
	if (left <= start && end <= right)
		return tree[n].first;
	return XOR(tree, n * 2, start, (start + end) / 2, left, right) ^ XOR(tree, n * 2 + 1, (start + end) / 2 + 1, end, left, right);
}
