...
void sum(vector<pair<ll, ll>>& tree, ll n, ll start, ll end, ll index)
{
	lazy(tree, n, start, end);
	...
	{
		sum(tree, n * 2, start, (start + end) / 2, index);
		sum(tree, n * 2 + 1, (start + end) / 2 + 1, end, index);
	}
}
